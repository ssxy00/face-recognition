# -*- coding: utf-8 -*-
# @Time        : 2021/1/23 21:17
# @Author      : ssxy00
# @File        : center_loss_trainer.py
# @Description :
...
class FRTrainer:
    def __init__(self, train_dataset, valid_dataset, model, args):
        ...
        self.centers = torch.load(args.center_path).to(self.device)
    ...
    def train_epoch_with_center_loss(self, epoch):
        ...
        for i, data in enumerate(tqdm_data):
            ...
            batch_loss = self.args.ce_factor * batch_ce_loss + self.args.lambda_factor * batch_center_loss
            batch_loss.backward()
            self.optimizer.step()
    ...
    def train(self):
        ...
        for epoch_idx in range(1, self.args.n_epochs + 1):
